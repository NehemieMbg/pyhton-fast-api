...
class BookRequest(BaseModel):
    id: Optional[int] = Field(title="id is not needed", default=None)  # don't forget default value if optional
...
@app.get("/books/", status_code=status.HTTP_200_OK)
async def get_book_by_rating(book_rating: int = Query(gt=1, lt=6)):
    books_to_return = []
...
async def post_book(request: BookRequest):
    new_book = Book(**request.dict())  # ? `**request.dict()`: convert request to Book object
